...
def run_week2_retrieval(
    ...
    model = TwoTowerRecommender(seed=seed, **config.get("model", {}))
...
def save_week2_results(results: dict[str, Any], artifacts_dir: str | Path) -> None:
    ...
    with (output / "week2_results.json").open("w", encoding="utf-8") as handle:
    ...
    pd.DataFrame(rows).to_csv(output / "week2_results.csv", index=False)
